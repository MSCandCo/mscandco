...
CREATE POLICY "Admins can view all profiles" ON user_profiles
  FOR SELECT USING (
    EXISTS (
      ...
      AND (
        auth.users.raw_user_meta_data->>'role' IN ('company_admin', 'super_admin') OR
        auth.users.user_metadata->>'role' IN ('company_admin', 'super_admin')
      )
    )
  );
...
SELECT id, email, first_name, last_name FROM user_profiles WHERE auth.uid() = id LIMIT 1;
