...
export default function ArtistEarnings() {
  ...
  const renderAdvancedEarnings = () => {
    const advancedMetrics = earningsData?.advancedMetrics || {
      ...
      <div className="space-y-8">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center">
              ...
              <span className="text-sm font-medium text-gray-700">
                Period: {earningsData?.dateRange?.period || '30d'}
              </span>
            </div>
            {earningsData?.dateRange?.startDate && earningsData?.dateRange?.endDate && (
              <div className="text-sm text-gray-600">
                {new Date(earningsData.dateRange.startDate).toLocaleDateString()} - {new Date(earningsData.dateRange.endDate).toLocaleDateString()}
              </div>
            )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-6">
          {Object.entries(advancedMetrics).map(([key, metric]) => (
            <div key={key} className="bg-white rounded-2xl shadow-lg border border-slate-200 p-6 text-center hover:shadow-xl transition-shadow">
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8">
          ...
          <div className="h-80">
            {earningsData?.platformRevenue?.length > 0 ? (
              <Line 
                data={{
                  ...
                  datasets: [{
                    ...
                    data: [
                      advancedMetrics.totalEarnings.value * 0.6,
                      advancedMetrics.totalEarnings.value * 0.7,
                      advancedMetrics.totalEarnings.value * 0.8,
                      advancedMetrics.totalEarnings.value * 0.9,
                      advancedMetrics.totalEarnings.value * 0.95,
                      advancedMetrics.totalEarnings.value
                    ],
                    borderColor: '#10B981',
              ...
              <div className="h-full flex items-center justify-center border-2 border-dashed border-gray-200 rounded-lg">
                <div className="text-center">
                  ...
                  <p className="text-gray-400 text-sm mt-2">Set up your earnings data to see trends</p>
                </div>
              </div>
            )}
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            {earningsData?.platformRevenue?.length > 0 ? (
              <div className="space-y-6">
                ...
                <div className="space-y-3">
                  {earningsData.platformRevenue.map((platform, index) => (
          ...
          <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-8">
            ...
            {earningsData?.territoryRevenue?.length > 0 ? (
              <div className="space-y-6">
                <div className="h-64">
                  <Bar data={{
                ...
                <div className="space-y-3 max-h-32 overflow-y-auto">
                  {earningsData.territoryRevenue.map((territory, index) => (
              ...
              <div className="flex space-x-1 bg-white p-1 rounded-xl shadow-sm border border-slate-200">
                ...
                >
                  <span className="flex items-center">
                    ...
                    Advanced Earnings
                    {hasProAccess ? (
                      <span className="ml-2 text-xs bg-green-500 text-white px-2 py-1 rounded-full opacity-90">Pro</span>
                    ) : (
            ...
            {activeTab === 'advanced' && !hasProAccess && (
              <div className="bg-white rounded-2xl shadow-lg border border-slate-200 p-12 text-center">
                ...
                <Link href="/artist/subscription">
                  <button className="bg-gradient-to-r from-green-600 to-emerald-600 text-white font-semibold py-4 px-8 rounded-xl hover:from-green-700 hover:to-emerald-700 transition-all transform hover:scale-105">
                    Upgrade to Pro
                  </button>
                </Link>
              </div>
