...
export default function CompanyAdminRequests() {
  const { user } = useUser();
  ...
  const loadAllRequests = async () => {
    try {
      ...
      const artistResponse = await fetch('/api/admin/artist-requests', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      
      // Load profile change requests
      const profileResponse = await fetch('/api/admin/profile-change-requests', {
        headers: { 'Authorization': `Bearer ${token}` }
      });
            ...
            {artistRequests.length === 0 ? (
              ...
              artistRequests.map((request) => (
                <tr key={request.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900">{request.label_admin_name}</div>
                    <div className="text-sm text-gray-500">{request.label_admin_email}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="grid grid-cols-2 gap-4 text-sm">
                    <div>
                      ...
                      <span className="ml-2 text-gray-900">{selectedRequest.artist_name}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Email:</span>
                      <span className="ml-2 text-gray-900">{selectedRequest.artist_email}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Invited by:</span>
                      <span className="ml-2 text-gray-900">{selectedRequest.label_admin_name}</span>
                    </div>
                    <div>
                      <span className="text-gray-500">Label Email:</span>
                      <span className="ml-2 text-gray-900">{selectedRequest.label_admin_email}</span>
                    </div>
                  </div>
                </div>
              ) : (
