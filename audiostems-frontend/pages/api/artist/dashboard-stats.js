  ...
  try {
    ...
    if (!userRole) {
      if (userEmail === 'info@htay.co.uk') {
        ...
        userRole = 'artist' // default for unknown users
      }
    }
    ...
    if (userRole !== 'artist') {
      console.log('❌ Access denied - expected artist, got:', userRole)
      return res.status(403).json({ 
        error: 'Artist access required',
        details: `Your role is '${userRole}', but 'artist' is required`
      })
    }
