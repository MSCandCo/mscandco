...
const Player = dynamic(() => import("./playlists/[id]").then(mod => mod.Player), { ssr: false });
...
function SongsPageContent() {
    ...
    const id = setTimeout(() => {
      setFilterQuery((query) => {
        ...
        if (checkedArtists.length) {
          query.filters.singers.id.$in = checkedArtists
            .filter((a) => a.filter === "include")
            .map((a) => a.id);
          query.filters.singers.id.$notIn = checkedArtists
            .filter((a) => a.filter === "exclude")
            .map((a) => a.id);
        } else {
          query.filters.singers.id.$in = [];
          query.filters.singers.id.$notIn = [];
        }
        ...
        query.filters.$or[1].singers.name.$containsi = searchQuery;
        return { ...query };
      });
    }, 750);
  ...
  const { data: { data: songs = [] } = {}, isLoading } = useSWR(
    apiRoute(
      `/songs?populate=deep,3&${qs.stringify(filterQuery, {
