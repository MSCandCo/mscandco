/**
 * Apollo Intelligence - Tool Executor
...
export async function executeToolCall(toolName, toolInput, userId) {
  ...
  try {
    switch (toolName) {
      ...
      case 'update_profile': {
        // First, get current profile to check locked status and track changes
        const { data: currentProfile } = await supabase
          .from('user_profiles')
          .select('artist_name, first_name, last_name, bio, phone, immutableDataLocked, dateOfBirth, nationality, city, postalCode')
          .eq('id', userId)
          .single();

        // Check if trying to update locked personal information
        const lockedFields = ['first_name', 'last_name', 'dateOfBirth', 'nationality', 'city', 'postalCode'];
        const attemptedLockedFields = lockedFields.filter(field =>
          toolInput[field] !== undefined ||
          (field === 'first_name' && toolInput.first_name !== undefined) ||
          (field === 'last_name' && toolInput.last_name !== undefined)
        );
        ...
        if (currentProfile?.immutableDataLocked && attemptedLockedFields.length > 0) {
          return {
            success: false,
            error: 'LOCKED_FIELDS',
            message: "I can't change that field directly - your personal information (name, date of birth, nationality, city, postal code) is locked for security. To update this, you'll need to submit a Profile Change Request through your profile page, and our team will review it. This keeps your account secure! 🔒",
