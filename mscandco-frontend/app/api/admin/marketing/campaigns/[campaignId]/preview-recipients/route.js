...
/**
 * GET /api/admin/marketing/campaigns/[campaignId]/preview-recipients
 * Preview which users would receive this campaign based on filters
 */
export async function GET(request, { params }) {
  try {
    ...
    const recipients = await buildRecipientQuery(supabaseAdmin, campaign.filters, limit)
    ...
    console.error('Error in GET /api/admin/marketing/campaigns/[campaignId]/preview-recipients:', error)
    return NextResponse.json(
...
async function buildRecipientQuery(supabaseAdmin, filters, limit = 100) {
  let query = supabaseAdmin
    .from('user_profiles')
    .select('id, email, display_name, first_name, last_name, role, city, nationality, created_at')
    .limit(limit)
  ...
  if (filters.lastLoginDays) {
    ...
    // We need to join with login_history to check last login
    // For now, we'll use last_active_at if it exists, or created_at as fallback
