...
export const POST = withApiRateLimit(async (request) => {
  ...
  const { moderationId, reason, notes } = body

  if (!moderationId || !reason) {
    return NextResponse.json({ error: 'moderationId and reason are required' }, { status: 400 })
  }

  // Call the reject_content function
