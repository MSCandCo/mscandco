...
export default function GhostLoginClient({ user }) {
  ...
  const fetchUsers = async (search) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Filter out super admins and format for display
        const filteredUsers = (data.users || []).filter(u => u.role !== 'super_admin').map(u => ({
          id: u.id,
          email: u.email,
  ...
  const fetchActiveSessions = async () => {
    try {
      ...
      const response = await fetch('/api/superadmin/ghostlogin', {
        headers: {
