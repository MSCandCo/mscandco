...
-- Recreate with explicit SECURITY INVOKER (or omit to use default)
CREATE VIEW public.email_marketing_stats
...
-- Recreate with explicit SECURITY INVOKER (or omit to use default)
CREATE VIEW public.cookie_consent_summary
WITH (security_invoker = true) AS
SELECT
...
CREATE VIEW public.user_wallet_balances
WITH (security_invoker = true) AS
SELECT 
