...
async function uploadFile(endpoint: string, filePath: string, metadata: any = {}) {
  const url = `${API_BASE_URL}${endpoint}`;
    ...
    tools: [
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            bio: { type: "string" },
            country: { type: "string", enum: COUNTRIES as any },
            city: { type: "string" },
            phone: { type: "string" },
            website: { type: "string" },
            socialLinks: {
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            copyright_type: { type: "string", enum: COPYRIGHT_TYPES as any, description: "Type of copyright" },
            upc: { type: "string", description: "UPC/EAN barcode (auto-generated if not provided)" },
            language: { type: "string", enum: LANGUAGES as any, description: "Primary language (ISO 639-1)" },
            content_rating: { type: "string", enum: CONTENT_RATINGS as any, default: "clean", description: "Content rating" },
            territories: { type: "array", items: { type: "string", enum: TERRITORIES as any }, description: "Distribution territories" },
            mood_tags: { type: "array", items: { type: "string", enum: MOOD_TAGS as any }, description: "Mood/vibe tags" },
          },
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            query: { type: "string" },
            genre: { type: "string", enum: MUSIC_GENRES as any },
            release_type: { type: "string", enum: RELEASE_TYPES as any },
            year: { type: "number" },
          },
          required: ["query"],
        },
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            audioFormat: { type: "string", enum: AUDIO_FORMATS as any, description: "Audio format" },
            title: { type: "string" },
            version: { type: "string", enum: TRACK_VERSIONS as any, description: "Track version type" },
            genre: { type: "string", enum: MUSIC_GENRES as any },
            language: { type: "string", enum: LANGUAGES as any, description: "Language (ISO 639-1)" },
            ...
            contributors: {
              ...
              items: {
                ...
                properties: {
                  name: { type: "string" },
                  role: { type: "string", enum: CONTRIBUTOR_ROLES as any },
                },
              },
            },
          },
      ...
      },
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            platforms: {
              type: "array",
              items: { type: "string", enum: PLATFORMS as any },
            },
      ...
      // ==========================================
      {
        ...
        inputSchema: {
          ...
          properties: {
            ...
            type: { type: "string", enum: NOTIFICATION_TYPES as any },
          },
        },
      },
      {
    ...
    switch (name) {
      ...
      case "get_releases": {
        ...
        const data = await apiCall(
          `/api/artist/releases-simple?status=${params.status || "all"}&limit=${params.limit || 50}&offset=${params.offset || 0}`
        );
        return {
      ...
      case "get_wallet_balance": {
        const params = args as any || {};
        const data = await apiCall(`/api/artist/wallet-simple?currency=${params.currency || "GBP"}`) as any;
        return {
          content: [{
            type: "text",
            text: JSON.stringify({
      ...
      case "get_platform_stats": {
        ...
        const stats = {
          total_releases: (releases as any).releases?.length || 0,
          live_releases: (releases as any).releases?.filter((r: any) => r.status === "live").length || 0,
          total_earned: (earnings as any).total_paid,
          pending_earnings: (earnings as any).total_pending,
          wallet_balance: (earnings as any).wallet_balance,
          total_streams: (analytics as any).total_streams || 0,
          top_platform: (analytics as any).top_platform || "N/A",
        };
      ...
      case "search_releases": {
        ...
        if (params.release_type) queryParams.append("type", params.release_type);
      ...
      case "get_notifications": {
        const params = args as any || {};
        const data = await apiCall(
          `/api/notifications?unread=${params.unread_only || false}&limit=${params.limit || 20}${params.type ? `&type=${params.type}` : ""}`
        );
        return {
          content: [{ type: "text", text: JSON.stringify(data, null, 2) }],
        };
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              artistId: (data as any).artistId,
              message: (data as any).exists
      ...
      case "submit_distribution": {
        ...
        const data = await apiCall("/api/v1/releases/submit", {
          ...
          body: JSON.stringify({
            ...
            artworkId: (artworkData as any).artworkId,
        ...
        return {
          content: [{
            ...
            text: JSON.stringify({
              ...
              releaseId: (data as any).releaseId,
              status: (data as any).status,
              expectedLiveDate: (data as any).expectedLiveDate,
              platforms: params.platforms,
              data,
            }, null, 2)
          }],
