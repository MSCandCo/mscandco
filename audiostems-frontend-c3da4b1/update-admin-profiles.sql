-- Update existing user profiles to admin roles
-- Replace the UUIDs with the actual ones from your auth.users table

-- Update Super Admin (replace the UUID)
...
-- Update Company Admin (replace the UUID)
UPDATE user_profiles 
SET 
...
-- (Use the actual UUIDs and emails from your auth.users)
...
SELECT 
  ...
  NOW()
FROM auth.users au
WHERE au.email = 'superadmin@mscandco.com'
...
SELECT 
  ...
  NOW()
FROM auth.users au
WHERE au.email = 'companyadmin@mscandco.com'
AND NOT EXISTS (SELECT 1 FROM user_profiles WHERE id = au.id);
