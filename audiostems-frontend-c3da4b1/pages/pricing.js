...
function Pricing() {
  ...
  // Handle subscription with Revolut (Phase 1) or Stripe fallback
  const handleSubscribe = async (plan) => {
    try {
      console.log('🚀 Starting subscription for plan:', plan.name);
      
      // Get the price ID based on plan and billing interval
      const priceId = getPriceIdForPlan(plan, chargingInterval);
      ...
      // Try Revolut first (Phase 1), fallback to Stripe
      const useRevolut = true; // Always use Revolut (Phase 1)
      
      if (useRevolut) {
        // Phase 1: Revolut checkout
        await handleRevolutCheckout(priceId, plan);
      } else {
  ...
  // Revolut checkout handler
  const handleRevolutCheckout = async (priceId, plan) => {
    const response = await fetch('/api/billing/create-revolut-checkout', {
      ...
      body: JSON.stringify({
        ...
        userId: user?.id,
        successUrl: `${window.location.origin}/billing?success=true&provider=revolut`,
        cancelUrl: `${window.location.origin}/pricing?canceled=true&provider=revolut`
      }),
    });
    ...
    if (data.url) {
      // Show fee savings before redirect
      const savings = data.planData?.revolutFees?.savings;
      if (savings) {
        const confirmed = confirm(`💰 Great choice! You'll save ${savings.savingsPercentage} on fees with Revolut!\n\nStripe would charge: £${savings.stripeFee}\nRevolut charges: £${savings.revolutFee}\nYour savings: £${savings.savings}\n\nContinue to checkout?`);
        if (!confirmed) return;
      }
  ...
  const handleStripeCheckout = async (priceId, plan) => {
    const response = await fetch('/api/billing/create-checkout-session', {
      ...
      body: JSON.stringify({
        ...
        userEmail: user?.email,
        userId: user?.id,
        successUrl: `${window.location.origin}/billing?success=true&provider=stripe`,
        cancelUrl: `${window.location.origin}/pricing?canceled=true&provider=stripe`
      }),
    });
  ...
  // Map plan and interval to price ID
  const getPriceIdForPlan = (plan, interval) => {
    const priceMap = {
      'Artist Starter': {
        monthly: 'price_1Rsf8jEbz7hJzxJVL4PddB68',
        yearly: 'price_1Rsf95Ebz7hJzxJVdsKFDu6v'
      },
      'Artist Pro': {
        monthly: 'price_1RsfC6Ebz7hJzxJVunfzoZUD', 
        yearly: 'price_1RsfCLEbz7hJzxJV1LnfFIcS'
      },
      'Label Admin Starter': {
        monthly: 'price_1RsfCvEbz7hJzxJV5Ev8mZk6',
        yearly: 'price_1RsfD7Ebz7hJzxJVmS7BGMGH'
      },
      'Label Admin Pro': {
        monthly: 'price_1Rsph0Ebz7hJzxJVEHqJ8sye',
        yearly: 'price_1RsphDEbz7hJzxJVk4smCHyg'
      }
    };
    
    return priceMap[plan.name]?.[interval];
  };
