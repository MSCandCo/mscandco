...
const ContributorVideoSection = () => {
  ...
  useEffect(() => {
    // Set the uploaded YAHWEH video as the hero video
    setCurrentVideo({
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="flex items-center space-x-4 text-sm opacity-90">
                <span className="font-medium">YAHWEH - Angeloh, CalledOut Music & Tbabz</span>
              </div>
            </div>
...
export default function LabelAdminRoster() {
  ...
  const loadRoster = async () => {
    try {
      ...
      console.log('Label Admin Roster response status:', response.status);
      
      if (response.ok) {
        const data = await response.json();
        console.log('Label Admin Roster data:', data);
        setRoster(data);
      } else {
        console.error('Label Admin Roster response not ok:', response.status, response.statusText);
        setRoster([]);
      }
    } catch (error) {
      console.error('Error loading label admin roster:', error);
      setRoster([]);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!validateISNI(formData.isni)) {
      showError('Invalid ISNI format. Must be 16 digits with last character being a digit or X.');
      return;
    }
    ...
    try {
      ...
      const url = editingContributor 
        ? `/api/labeladmin/roster/${editingContributor.id}`
        : '/api/labeladmin/roster';
      ...
      if (response.ok) {
        ...
        showSuccess(`Contributor ${editingContributor ? 'updated' : 'added'} successfully!`);
      }
    } catch (error) {
      console.error('Error saving contributor:', error);
      showError('Failed to save contributor. Please try again.');
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    const contributorName = contributor ? contributor.name : 'this contributor';
    
    confirmDelete(contributorName, async () => {
      try {
        const response = await fetch(`/api/labeladmin/roster/${id}`, {
          method: 'DELETE'
        });
        if (response.ok) {
          await loadRoster();
