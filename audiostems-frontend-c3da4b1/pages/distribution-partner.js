...
// Removed SWR and apiRoute imports - using Supabase directly
...
function DistributionPartnerDashboard() {
  ...
  useEffect(() => {
    if (isLoading) return;
    if (!isAuthenticated || !user || getUserRole(user) !== 'distribution_partner') {
      router.push("/");
    }
  }, [isAuthenticated, isLoading, user, router]);

  // Removed Strapi SWR calls - now using empty state for ground zero
